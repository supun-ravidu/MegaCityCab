...
public class PrintBillServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (orderNumber == null || orderNumber.trim().isEmpty()) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Order number is required");
            return;
        }
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                int width = 600;
                int height = 500;  // Increased height to accommodate all details
                ...
                // Set the header to indicate that the response is a downloadable file
                response.setHeader("Content-Disposition", "attachment; filename=booking_bill_" + orderNumber + ".jpg");

                OutputStream out = response.getOutputStream();

                // Write the image to the output stream in JPEG format
